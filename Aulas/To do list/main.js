...
function createLi(){ // VAI CRIAR UM ELEMENTO <LI>
    const li = document.createElement('li')
    return li
}

function createTask(inputText){ // VAI CRIAR UMA TAREFA
    const li = createLi()
...
function clearInput(){ // VAI LIMPAR O INPUT
    inputTask.value = ''
    inputTask.focus()
}

function createDeleteButton(li) { // VAI CRIAR UM BOTAO DE APAGAR
...
function saveTask(){ // VAI SALVAR AS TAREFAS
...
function redeemTasks(){ // VAI RECUPERAR AS TAREFAS AO RECARREGAR A PAGINA
...
btnTask.addEventListener('click', (e) => { // EVENTO QUE CLICA NO BOTAO DE ADD. TAREFA
    if(!inputTask.value) return
    createTask(inputTask.value)
})

inputTask.addEventListener('keypress', (e) => {  // EVENTO QUE AO APERTAR ENTER VAI ADD. TAREFA
    if(!inputTask.value) return
    if (e.keyCode === 13){
        createTask(inputTask.value)
    }
...
document.addEventListener('click', (e) => { // BOTAO DE APAGAR A TAREFA
